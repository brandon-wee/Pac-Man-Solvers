...
class Q2Agent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state: GameState):
        ...
        for action in legal_actions:
            q_value = self.getQValue((x, y), action)
            if q_value > max_q:
                max_q = q_value
    ...
    def update(self, state: GameState, action, nextState: GameState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        current_x, current_y = state.getPacmanPosition()
        action_index = self.getActionIndex(action)

        current_q = self.getQValue((current_x, current_y), action)

        x, y = nextState.getPacmanPosition()
        if (x, y) in self.food_locations or (x, y) in nextState.getGhostPositions():
            next_value = 0.0
        else:
            next_value = self.computeValueFromQValues(nextState)

        updated_q = current_q + self.alpha * (reward + self.discount * next_value - current_q)
        self.Q_values[current_x][current_y][action_index] = updated_q
